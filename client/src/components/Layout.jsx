...
const PRIMARY_LINKS = [
  ...
  { to: '/dashboard/approvals', label: 'Approvals', roles: ['SuperAdmin', 'Admin', 'BranchManager'] },
...
export default function Layout() {
  ...
  const visibleLinks = PRIMARY_LINKS.filter((l) => !l.roles || l.roles.includes(employee.role));
      ...
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-paper-line shadow-sm">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            <div className="hidden sm:block leading-none">
              ...
              <p className="font-mono text-[9px] tracking-widest text-brass-dark uppercase mt-0.5">Core Banking</p>
            </div>
          </div>
              ...
              <div className="relative" ref={adminRef}>
                <button
                  ...
                  className={`flex items-center gap-1 px-3.5 py-2 text-sm font-medium rounded-md transition-colors duration-150 ${
                    adminOpen ? 'text-ink-900 bg-ink-50' : 'text-slate-soft hover:text-ink-900 hover:bg-ink-50'
                  }`}
                >
                  Admin
                  <svg width="10" height="10" viewBox="0 0 12 12" className={`transition-transform duration-200 ${adminOpen ? 'rotate-180' : ''}`}>
                      ...
                      <NavLink
                        ...
                        className={({ isActive }) =>
                          `block px-4 py-2.5 text-sm transition-colors ${
                            isActive ? 'bg-brass-100 text-brass-dark font-medium' : 'text-slate hover:bg-ink-50'
                          }`
                        }
                      >
                        {l.label}
            ...
            <div className="relative hidden sm:block" ref={userRef}>
              ...
              >
                <div className="text-right leading-none hidden md:block">
                  ...
                  <p className="font-mono text-[10px] text-brass-dark uppercase tracking-wide mt-0.5">{employee.role}</p>
                </div>
                <div className="w-9 h-9 rounded-full bg-ink-900 border-2 border-brass/40 flex items-center justify-center shrink-0">
                  <span className="font-mono text-xs text-brass font-semibold">{initials(employee.name)}</span>
                </div>
              </button>
              ...
              {userOpen && (
                <div className="animate-slide-down absolute top-full right-0 mt-2 w-56 bg-white border border-paper-line rounded-lg shadow-xl py-1.5 overflow-hidden">
                  <div className="px-4 py-3 border-b border-paper-line">
                    <p className="text-sm font-medium text-ink-900">{employee.name}</p>
                    <p className="text-xs text-slate-soft">{employee.branchName || 'All Branches'}</p>
                  </div>
                  <NavLink
        ...
        {mobileOpen && (
          <div className="lg:hidden animate-slide-down border-t border-paper-line bg-white px-4 py-3 space-y-1">
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `block px-3 py-2.5 rounded-md text-sm font-medium ${
                    isActive ? 'bg-brass-100 text-brass-dark' : 'text-slate hover:bg-ink-50'
                  }`
                }
              >
                {link.label}
            ...
            {isSuperAdmin && ADMIN_LINKS.map((l) => (
              <NavLink
                key={l.to}
                to={l.to}
                onClick={() => setMobileOpen(false)}
            ...
            <div className="pt-2 mt-2 border-t border-paper-line flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-ink-900">{employee.name}</p>
                <p className="font-mono text-[10px] text-brass-dark uppercase">{employee.role}</p>
              </div>
